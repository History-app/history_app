...
class _OpenUpwardsPageTransition extends StatelessWidget {
  ...
  static final Tween<double> _scrimOpacityTween = Tween<double>(begin: 0, end: 0.25);

  // Used by all of the transition animations.
  static const Curve _transitionCurve = Cubic(0.20, 0, 0, 1);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final clipAnimation = Tween<double>(begin: 0, end: size.height).animate(primaryAnimation);

        final opacityAnimation = _scrimOpacityTween.animate(primaryAnimation);
        final primaryTranslationAnimation = _primaryTranslationTween.animate(primaryAnimation);

        final secondaryTranslationAnimation = _secondaryTranslationTween.animate(
          CurvedAnimation(
  ...
  Widget build(BuildContext context) {
    return DualTransitionBuilder(
      ...
      forwardBuilder: (BuildContext context, Animation<double> animation, Widget? child) {
        return _ZoomEnterTransition(animation: animation, child: child!);
      },
      reverseBuilder: (BuildContext context, Animation<double> animation, Widget? child) {
        return _ZoomExitTransition(animation: animation, reverse: true, child: child!);
      },
      child: DualTransitionBuilder(
        animation: ReverseAnimation(secondaryAnimation),
        forwardBuilder: (BuildContext context, Animation<double> animation, Widget? child) {
          return _ZoomEnterTransition(animation: animation, reverse: true, child: child!);
        },
        reverseBuilder: (BuildContext context, Animation<double> animation, Widget? child) {
          return _ZoomExitTransition(animation: animation, child: child!);
        },
        child: child,
      ),
    );
...
class _ZoomEnterTransition extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fadeTransition = reverse
        ? kAlwaysCompleteAnimation
        : _fadeInTransition.animate(animation);

    final scaleTransition = (reverse ? _scaleDownTransition : _scaleUpTransition).animate(
      animation,
    );
...
///    which defines a default page transition.
...
abstract class PageTransitionsBuilder {
  ...
  /// Wraps the child with one or more transition widgets
  /// which define how [route]
...
class PageTransitionsTheme with Diagnosticable {
  ...
  const PageTransitionsTheme({required Map<TargetPlatform, PageTransitionsBuilder> builders})
    : _builders = builders;
  ...
  ///
  /// [buildTransitions] delegates to this method.
  Widget buildTransitions<T>(
    PageRoute<T> route,
    BuildContext context,
    Animation<double> animation,
    ...
    final matchingBuilder = builders[platform] ?? const FadeUpwardsPageTransitionsBuilder();
